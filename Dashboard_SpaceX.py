...
def select(inputt):
    if inputt == 'All Sites':
        ...
        fig = px.pie(new_df, values='class', names='Launch Site', title='Total Success Launches by Site', hole=.3)
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == inputt]["class"].value_counts().to_frame()
        new_df["name"] = ["Failure", "Success"]
        fig = px.pie(new_df, values='class', names='name', title='Total Success Launches for ' + inputt, hole=.3)
    return fig
# TASK 4:
...
def scatter(input1, input2):
	...
	new_df = spacex_df    
        ...
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category", size='Payload Mass (kg)')
    else:
